...
2. 在配置页面按需要打开：
   ...
   - **公告通知** -> **RM 公告监控** -> **开启 RM 公告监控**
   - **赛事通知** -> **RM 赛事监控** -> **开启 RM 赛事监控**
   - **论坛开源监控** -> **论坛抓取** -> **开启论坛开源监控**

`RM订阅通知` 只负责把当前群/会话加入通知接收列表；是否真的推送公告、赛事或论坛开源内容，由对应监控开关决定。改完配置后建议重载或重启插件，管理员可发送 `RM监控状态` 检查后台任务和订阅数量。需要立即验证时，管理员可以发送 `RM公告检查` 或 `RM赛事检查`。
